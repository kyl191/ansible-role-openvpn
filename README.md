...
|------------------------------|---------|-------------|-------------------|-------------------------------------------------------------------------------|
| openvpn_ci_build             | boolean | true, false | false             | Set to true to indicate a CI build. Do not use, will be deprecated in a later version. |
...
| openvpn_selinux_module       | string  |             | my-openvpn-server | Name of the legacy custom SELinux module to unload on upgrade. Only relevant if you previously used `openvpn_selinux_use_semanage: false` with a custom module name. |
...
| openvpn_fetch_client_configs_dir          | string  |             | /tmp/ansible-openvpn-certs | If openvpn_fetch_client_configs is true, the local directory to download the client config files into. If security is a concern, use something under the user's directory like `"{{ lookup('env', 'HOME') }}/ovpn-client-configs"`                                     |
...
| openvpn_custom_ipv6_dns     | list[string] |                   | Cloudflare/Google public IPv6 nameservers    | List of IPv6 DNS servers, only applied if `openvpn_set_dns` is set to true                                                                           |
...
| openvpn_manage_sysctl       | boolean      | true, false       | true                       | Allow playbook to enable IPv4/IPv6 forwarding via sysctl. Set to false if it's already set, or can't be written by Ansible.                          |
...
| openvpn_redirect_gateway    | string      |       | `def1 bypass-dhcp ipv6` | Sets the `redirect-gateway` value that gets pushed to clients. Set to an empty string to disable sending it. Both the IPv4 and IPv6 flags are pushed unconditionally, even if one address family's tunnel is disabled below - this is a kill switch: it blackholes that family's traffic in the client rather than letting it leak out over the client's local connection. |
...
| openvpn_server_ipv6_network | string       |                   | `fdbf:dd0d:1a49:2091::/64` | The network address and prefix of an IPv6 network to assign to clients. Set to an empty string to disable IPv6 tunnel support entirely. |
| openvpn_server_netmask      | string       |                   | 255.255.255.0              | Netmask of the private network     |
| openvpn_server_netmask_cidr      | string       |                   | Determined at runtime from `openvpn_server_netmask` | CIDR suffix to use in firewall rules |
| openvpn_server_network      | string       |                   | 10.9.0.0                   | Private network used by OpenVPN service. Set to an empty string to disable IPv4 tunnel support entirely (requires a host with no default IPv4 route to also skip IPv4 firewall rules; see `openvpn_snat_source_ipv4`) |
...
| openvpn_conf_group                 | string  |             | root                                             | Group owner of configuration files.                                                                                                                                           |
| openvpn_conf_user                  | string  |             | root                                             | User owner of configuration files.                                                                                                                                            |
...
| openvpn_script_group               | string  |             | root                                             | Group owner of script files.                                                                                                                                                  |
| openvpn_script_user                | string  |             | root                                             | User owner of script files.                                                                                                                                                   |
...
| openvpn_use_logrotate    | boolean |         | true                                                                                                        | Whether or not to use logrotate.                                                                          |
