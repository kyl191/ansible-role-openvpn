...
## Removed: legacy SELinux module compilation

The `openvpn_selinux_use_semanage: false` path — which compiled and installed a custom SELinux
TE module using `checkmodule`/`semodule_package`/`semodule` — has been removed. The
`openvpn_selinux_use_semanage` variable no longer exists.

The role now always uses `semanage port` (via `community.general.seport`) to configure SELinux
port permissions. This has been the default since 3.1.0.

On upgrade, the role will automatically detect and unload any previously-installed custom module
(controlled by `openvpn_selinux_module`, default: `my-openvpn-server`).
...
2. DH Param cleanup
3. Better `uv`  management of dependencies

## Planned Deprecations

1. Delete support for compiling and installing the custom OpenVPN SELinux module
2. Delete parameters
  a. `openvpn_compression` - [VORACLE attack](https://community.openvpn.net/Security%20Announcements/VORACLE) means it should stay unset
  b. `openvpn_ci_build` - overloaded to gate sysctl & firewall settings; use the new `openvpn_manage_sysctl` and/or `openvpn_manage_firewall_rules` instead.
...
5. Drop `ansible.utils` Ansible Galaxy dependency

# Version 3.2.0 (2026-07-23)

## Behaviour Changes

* Remove `AES-256-CBC` from default ciphers
  * This was blocking Data Channel Offload; expected to be a no-op since OpenVPN 2.5+ support the remaining `AES-256-GCM:AES-128-GCM` cipher list
  * Restore old behaviour with `openvpn_cipher: AES-256-GCM:AES-128-GCM:AES-256-CBC`
* New validation/correctness checks
  * (Hard failure) At least one of `openvpn_server_network`/`openvpn_server_ipv6_network` must be set
  * Check generated certificate names are acceptable under TLS rules limiting names to <=64 characters
  * Warn about deprecated openvpn_ci_build use
  * Stricter parameter parsing: more boolean coercion & non-empty string checks
* Not having a IPv4/v6 default route set will now crash the playbook with a descriptive error message if SNAT is required with ufw/iptables
  ...
  * Previously undocumented `openvpn_lan_source_ip` parameter has been replaced by `openvpn_snat_source_ipv4`
* `openvpn_fetch_client_configs_dir` is now set to `/tmp/ansible-openvpn-certs`
  * Restore the old behaviour with `openvpn_fetch_client_configs_dir: /tmp/ansible`
* When limiting plays with `--tags=`, OS specific variable resolution is now performed

## Improvements

* IPv6-only support preview
  * Generated `server.ovpn` no longer hardcodes server IPv4 address
  * Disable IPv4 with `openvpn_server_network: ""`
  * Requires `openvpn_server_ipv6_network` to be set
* Firewalld zone usage now uses the default zone for the system instead of the zone associated with the interface that has the default route on it ([there is some magic behind this](https://medium.com/opsops/ansible-default-ipv4-is-not-what-you-think-edb8ab154b10))
  * Override the default zone by setting `openvpn_firewalld_zone`
* New `netmask_to_cidr` filter replacing the undeclared `netaddr` Python library dependency through `ansible.utils`
  * `openvpn_server_netmask_cidr` automatic calculation should no longer fail if the control node doesn't have it installed
* New parameters
  * `openvpn_ca_cert_validity_days`/`openvpn_cert_validity_days`: Customize the length of time certs should be valid for
  * `openvpn_fetch_client_configs_per_user_dir`: download client configs to a flat (single) directory as opposed to a directory for each client
  * `openvpn_ovpn_server_name`: Customize server name used in the downloaded certificate file names
  * `openvpn_manage_sysctl`: Allow OpenVPN to manage sysctls; functional replacement for `openvpn_ci_build`
  * Already mentioned: `openvpn_snat_source_ipv4/v6`, `openvpn_firewalld_zone`
  * More customization of CN names allowed, read [the Certificate Common Names](https://github.com/kyl191/ansible-role-openvpn#certificate-common-names-cn) for more information
    * Note: Changing fields on an already established set of clients & servers will require reissuing all certs
  * Documented some undocumented existing parameters

## Developer Improvements

* CLAUDE.md file for agents to reference
* Cleaner firewall detection
* Fix for Fedora/derivatives failing to start dbus and subsequently failing firewalld communication
* New CI tests for IPv4/v6 routing weirdness
* End to end test script now tests x86_64, ARM, ipv4-only and ipv6-only machines
* CI version bumps
