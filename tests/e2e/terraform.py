...
def terraform_apply(tf_dir: Path, var_file: str) -> bool:
    ...
    raising) on failure so the caller can still continue on to the next scenario."""
...
def terraform_destroy(tf_dir: Path, var_file: str) -> None:
    ...
    if result.returncode != 0:
        logger.error(f"Terraform destroy failed for {var_file} - check {tf_dir} state manually.")
